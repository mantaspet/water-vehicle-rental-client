<template>
  <div>
    ...
    <div style="display: flex">
      <div v-show="$userRole('admin')" id="status" class="mdc-select">
        <i class="mdc-select__dropdown-icon"></i>
        <select v-model="selectedEmployee" class="mdc-select__native-control" @change="getEmployeeTasks">
          <option :value="null"></option>
          <option v-for="employee in $store.getters.employees" :key="employee.userId" :value="employee">{{ `${employee.firstName} ${employee.lastName}` }}</option>
        </select>
